...
This skill is the single source of truth for wireframe round caps.

## Rendering
Render the HTML below via the inline HTML widget tool (the client's
show-widget capability) — NEVER paste the HTML as plain text into the chat;
it will not render.

## Render rules
- Match every input, output, and control named in the spec — exactly
- Flat components only: labels, inputs, buttons, result cards
- Realistic domain placeholder values (no lorem ipsum), but ALL numbers are
  illustrative — the badge line says so, so users don't anchor on them as
  promises
...
<div style="background: var(--color-background-secondary); border-radius: var(--border-radius-lg); padding: 1.5rem;">
  <div style="display:flex; align-items:center; gap:8px; margin-bottom:1.5rem;">
    ...
    <span style="font-size:13px; color: var(--color-text-secondary);">Layout preview only — not working code, numbers are illustrative</span>
  </div>
...
```
Layout preview only — no logic yet, numbers are illustrative. Match what you had in mind?
If yes I'll kick off the build; if not, tell me what to change.
```

## Response handling — max 5 rounds
Confirm → orchestrator appends the agreed layout to the spec, then creates the
issue (adlc-generate label) → Phase 3.
Change → layout-only: re-render · scope/spec-level: update spec then re-render.
Increment round on each change.
Round 4: include a heads-up with the re-render — "One more revision after this
and then we build; everything else can be adjusted on the live preview."
...
## Exit
Confirmed OR 5 rounds → orchestrator appends Layout section + creates GitHub
issue → Phase 3.
