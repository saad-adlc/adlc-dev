...
### WS10 — Enforce the full merge gate  *(CI gate LIVE + validated 2026-06-25 on PR #55; `adlc/business-approval` requirement deferred — see below)*
...
**Done + validated (2026-06-25):**
- ✅ ① upsert script + tests (6/6) — `933d519`;  ✅ ② all head-movers rewired — `8e0fdb8` + `8116e3e`;  ✅ ③ monitor — adlc-standards `59c06cf` (PR #5);  ✅ docs — merged via **PR #53**.
- ✅ ④ **Validation (PR #55, issue #54 "Unit converter"):** the PR head is the **single code commit** (`feat(…)`) — **no `[skip ci]` commit moved it**; `CI — Node/React` = success and `adlc/business-approval` = pending **both sit on the head**; the `<!-- adlc-status -->` status comment updated in place to `preview-deployed` and the dedicated 🔍 preview comment posted the live URL.
- ✅ ⑤ (partial) **`CI — Node/React` added to branch-protection required checks** (alongside the two CodeQL `Analyze`); `enforce_admins` + 0-review-count preserved. A PR with red/absent CI can no longer merge.
- ⚠️ **Caveat found 2026-06-25 (the D-4 rule, again):** a PR created by a **non-triggering token** does not fire `adlc-ci`, so `CI — Node/React` is **absent** and the required check blocks it. Observed when the **agent** opened a test docs PR (#56) via its `gho_` OAuth token — no `pull_request` workflow fired (only CodeQL, which triggers separately). **Real merge paths are unaffected:** your PRs and the pipeline's PAT-created PRs both fire `adlc-ci` (#53 user docs PR → `CI — Node/React` **SKIPPED**; #55 pipeline workspace PR → **SUCCESS**). **✅ Confirmed (2026-06-25, user-pushed PR #56):** a **SKIPPED** `CI — Node/React` *satisfies* the required check — after a ~40s GitHub recompute lag, #56 settled to `mergeState=UNSTABLE` (mergeable), not `BLOCKED`. So the gate is **safe for docs / non-workspace PRs**. Residual caveat: agent-created PRs (non-triggering token) don't fire `adlc-ci`, so the agent can't self-validate gate changes — use a user/pipeline PR.
- ⏳ **`adlc/business-approval` requirement DEFERRED (lockout):** it posts the gating status **only** on `feature/issue-*` branches (`adlc-business-approval.yml:18`), so requiring it would block every **manual** PR to `main` (status never posted → permanently blocked, even for admins with `enforce_admins`). **Follow-up:** make `adlc-business-approval` auto-pass non-`feature/issue-` PRs, then add `adlc/business-approval` to required checks → fully closes D-8 (the approval half of no-direct-merge). *(Decision 2026-06-25: ship the CI gate now; do the business-approval requirement as the scoped follow-up.)*
...
- **WS10 — CI GATE LIVE + validated (2026-06-25, PR #53 merged; validated on PR #55):** status decoupled to a single in-place PR comment (marker `<!-- adlc-status -->`); all `[skip ci]` head-movers rewired (incl. the legacy-generate `pr-open` the dep-map had missed); **`CI — Node/React` now a required check**. **Deferred:** `adlc/business-approval` requirement — it posts only on `feature/issue-*` PRs, so requiring it would lock manual PRs to `main`; follow-up makes it manual-PR-safe then requires it (closes D-8's approval half). Pairs with the distinct-bot-identity follow-up (D-7) for required *human* approval. See §4 WS10.
