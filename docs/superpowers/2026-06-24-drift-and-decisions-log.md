...
| D-8 | Required checks | CI + approval gate the merge | **WS10 implemented** (`feature/ws10-merge-gate`, 2026-06-25): live status decoupled from the branch to one in-place PR comment; **all** `[skip ci]` head-movers rewired (incl. the legacy-generate `pr-open` the dep-map missed). **Pending:** merge → test-generate → add `CI — Node/React` + `adlc/business-approval` to required checks. *(Was: only CodeQL required; those two deferred.)* | `[skip ci]` `status.json` commits moved the PR head past those checks (moving-head) → couldn't be required. Fixed by the comment channel; the rewired `pull_request`/`workflow_run` workflows take effect only once on `main` |
...
- ~~**Decouple `status.json` from the PR branch**~~ → **WS10 implemented** (`feature/ws10-merge-gate` + adlc-standards `feature/ws10-bootstrap-monitor`, 2026-06-25): post-PR status → an in-place PR comment (marker `<!-- adlc-status -->`). **Remaining to fully close D-8:** merge → one test-generate (confirm the head carries the two checks) → add **`CI — Node/React` + `adlc/business-approval`** as required checks.
