...
| D-8 | Required checks | CI + approval gate the merge | **WS10 shipped + validated** (2026-06-25; PR #53 merged, validated on PR #55): status decoupled to one in-place PR comment, all `[skip ci]` head-movers rewired (incl. the legacy-generate `pr-open` the dep-map missed), and **`CI — Node/React` is now a required check**. **Remaining:** `adlc/business-approval` requirement **deferred** — it posts only on `feature/issue-*` PRs, so requiring it would lock manual PRs to `main`; a follow-up makes it manual-PR-safe, then it becomes required (final close of D-8). *(Was: only CodeQL required.)* | moving-head fixed (validated: PR #55 head = code commit, both checks on it). CI gate enforced now; the approval half waits on the manual-PR exemption to avoid a solo-maintainer lockout |
...
- ~~**Decouple `status.json` from the PR branch**~~ → **WS10 done + validated** (PR #53 merged, validated on PR #55, 2026-06-25): post-PR status → an in-place PR comment (marker `<!-- adlc-status -->`); **`CI — Node/React` is now a required check**. **Remaining to fully close D-8:** make `adlc-business-approval` post on manual (non-`feature/issue-`) PRs, then add **`adlc/business-approval`** as a required check (deferred to avoid locking manual PRs to `main`).
